    ...
    return (
        ...
        >
            ...
            <motion.div 
                ...
                className="bg-white dark:bg-gray-800 rounded-3xl overflow-hidden max-w-5xl w-full shadow-2xl relative z-10 flex flex-col md:flex-row h-fit max-h-[90vh]"
            >
                <button 
                    onClick={onClose}
                ...
                <div className="md:w-[60%] relative group min-h-[300px] md:min-h-[450px]">
                    <img src={project.imageUrl} alt={project.title} className="w-full h-full object-cover" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
                </div>

                <div className="md:w-[40%] p-8 md:p-10 flex flex-col max-h-[90vh] overflow-y-auto">


                    <div className="flex-grow">
                        <span className="inline-block px-3 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 text-xs font-bold rounded-full mb-4 uppercase tracking-widest">
                            {project.category}
                        </span>
                        <h2 className="text-3xl font-extrabold text-gray-800 dark:text-white mb-6 leading-tight">{project.title}</h2>
                        
                        <div className="space-y-6">
                            <div>
                                <h4 className="text-sm font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-2">The Project</h4>
                            ...
                            <div>
                                <h4 className="text-sm font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-3">Technologies</h4>
                                <div className="flex flex-wrap gap-2">
                                    {project.tech.map((t, i) => (
                                        <span key={i} className="px-3 py-1.5 bg-gray-50 dark:bg-gray-700/50 text-gray-600 dark:text-gray-300 rounded-lg text-sm font-medium border border-gray-100 dark:border-gray-600">
                                            {t}
                                        </span>
                                    ))}
                                </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-10">
                        <a 
                            ...
                            className="flex items-center justify-center gap-2 bg-gray-800 dark:bg-gray-700 text-white font-bold py-3.5 rounded-2xl hover:bg-gray-900 dark:hover:bg-gray-600 transition-all shadow-lg"
                        >
                        ...
                        {project.hasDemo && (
                            <a 
                                ...
                                target="_blank" 
                                rel="noopener noreferrer"
                                className="flex items-center justify-center gap-2 bg-blue-600 dark:bg-blue-500 text-white font-bold py-3.5 rounded-2xl hover:bg-blue-700 dark:hover:bg-blue-400 transition-all shadow-lg shadow-blue-500/20"
                            >
...
const ProjectCard = ({ title, category, description, imageUrl, tech, hasDemo, demoLink, codeLink, onClick }) => (
    ...
    >
        <div className="relative overflow-hidden aspect-[4/3]">
            <img src={imageUrl} alt={title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" onError={(e) => { e.target.onerror = null; e.target.src = 'https://placehold.co/400x300/E2E8F0/4A5568?text=Image+Error'; }} />
        ...
        <div className="p-6 flex flex-col flex-grow">
            <h3 className="text-lg font-extrabold text-gray-800 dark:text-white mb-4 group-hover:text-blue-600 transition-colors leading-tight min-h-[3rem] flex items-center">{title}</h3>


            <div className="flex flex-wrap gap-1.5 mt-auto">

                {tech.slice(0, 3).map((t, i) => (
                    <span key={i} className="px-2 py-1 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-md text-[9px] font-bold uppercase tracking-wider">
                        {t}
                    </span>
                ))}
                {tech.length > 3 && (
                    <span className="px-2 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-md text-[9px] font-bold">
                        +{tech.length - 3}
                    </span>
                )}
            </div>
